...
const {
  ...
  uploadTask,
  upload,
} = useStorageFile(uploadedFileRef)
...
<template>
  <div class="flex">
    <button class="self-start w-full p-2 border border-gray-400 rounded-lg hover:bg-gray-100" type="button"
      @click="open({ accept: 'image/*', multiple: false }); hasUploaded = false" :disabled="!!uploadTask">
    ...
    <img :src="imageUrl" v-if="!!imageUrl" class="w-1/2 ml-4 rounded-lg max-w-40" />
  </div>
</template>
